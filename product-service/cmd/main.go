...
func main() {
	...
	productSvc := product.NewService(ctx, db)
	productServer := server.NewServer(productSvc)

	gServer := grpc.NewServer()
	pb.RegisterProductServiceServer(gServer, productServer)
	...
	log.Println("Server started and listening on port: 50052")
